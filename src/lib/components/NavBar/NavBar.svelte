...
<section class="text-white">
	<ContentLayout>
		<section class="flex py-5 sm:py-11 sm:px-16 items-center">
			...
			<p class=" uppercase text-xl sm:text-3xl lg:text-5xl font-bold">$Router</p>

			<ul class=" md:flex hidden font-medium gap-5 ml-auto text-base lg:text-xl">
				{#each navItem as item}
					<!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
					<li on:keydown on:click={(e) => handleClick(item.slug)} class="uppercase cursor-pointer">
						{item.name}
					</li>
				{/each}
			</ul>
			...
			>
				...
				>
					{#each navItem as item}
						...
						<li
							on:keydown
							on:click={(e) => handleClick(item.slug)}
